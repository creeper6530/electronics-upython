...
# running "from pio_uart import *" and then simply using "function(args)".
...
def receive(len): # Pull "len" amount of characters out of the FIFO, return them as one string
    global fifo_rx
    out = ""
    for _ in range(len):
...
if SELFTEST:
    transmit("SELFTEST SUCCESS\x04")
    sleep_ms(500)
    print(receive(8), end="")
    print(receive_until("\x04")[:-1])
